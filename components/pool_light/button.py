...
CONFIG_SCHEMA = button.BUTTON_SCHEMA.extend({
    ...
    cv.Optional(CONF_MODE, default='poolandspa'): cv.one_of(*MODE_VALUES.keys(), lower=True),
...
async def to_code(config):
    ...
    mode_str = config.get(CONF_MODE, 'poolandspa').lower()
    ...
    cg.add(var.set_cc1101_module(config[CONF_CC1101_MODULE]))
